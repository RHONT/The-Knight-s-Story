                ...
                case ("2"):
                    if (buf.money>400) {
                        ...
                        System.out.println("У вас не нашлось нужной суммы. Жулики забрали все что у вас есть, включая ваш инвентарь" +
                                ...
                                "меткость составляет:  " + buf.chance_to_attack);
                        buf.param_inventory=Arrays.stream(buf.param_inventory).map(e->{e=0; return e;}).toArray();
                        buf.money=0;
                        buf.event_on_bridge=true;
                        break;
                    }
...
//        System.out.println(path);
    ...
    void swith_for_halt(){
        ...
        switch (buf_str) {
            ...
            case "3":
                if (halt_craft) System.out.println("На привале можно починить лишь один элемент брони. Вы уже этим воспользовались");
                LABEL_1:
                while (!halt_craft) {
                    ...
                    switch (buf_str_for_craft) {
                        case "1": {defense[0]+=30; halt_craft=true; System.out.println("Доспех починен! Что выберете еще?"); print_defense();break;}
                        case "2": {defense[1]+=30; halt_craft=true; System.out.println("Доспех починен! Что выберете еще?"); print_defense();break;}
                        case "3": {defense[2]+=30; halt_craft=true; System.out.println("Доспех починен! Что выберете еще?"); print_defense();break;}
                        case "4": {defense[3]+=30; halt_craft=true; System.out.println("Доспех починен! Что выберете еще?"); print_defense();break;}
                        default:
...
class Knight extends Humanoid implements Humanoid_ability{
    ...
    public void Attack(Humanoid a){
        ...
        System.out.println("Атакуй! 1 - голова "+ (chance_to_attack-10)+"% | 2 -тело "+ (chance_to_attack)+
                ...
                "% | Слово - Щит (защита) или Молотов (атака)");
...
//                    switch (buf_war) {                        // этот код я вынужден был часто повторять, поэтому вынес его в отдельную функцию Calculate_chance_attack
//                        case 1: {buf_chance-=10; break;}
//                        case 2: {buf_chance-=0; break;}
//                        case 3: {buf_chance-=20; break;}
//                        default: {buf_chance-=20; break;}
//                    }
